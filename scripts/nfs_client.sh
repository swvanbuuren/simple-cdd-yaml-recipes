...
for mount_point in $MOUNT_POINTS; do
    ...
    # modify /etc/fstab
    echo "${SERVER_IP}:/mnt/${mount_point} /mnt/nas/${mount_point} nfs nofail,x-systemd.automount,x-systemd.requires=network-online.target,x-systemd.device-timeout=10s 0 0" >> /etc/fstab
done
